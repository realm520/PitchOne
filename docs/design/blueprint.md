...
本文档描述 PitchOne 去中心化链上体育预测平台的架构设计与实现。核心目标：**全链上结算、可组合 AMM、去中心化预言机 + 争议仲裁、资金与风控自动化、可扩展与低成本结算**。
...
PitchOne 以 **4 种定价引擎 + 7 种市场模板 + ERC-1155 头寸代币化 + UMA 乐观式预言机 + 去中心化仲裁** 为核心，辅以 **ERC-4626 流动性金库、Basket 串关系统、Merkle 奖励分发、参数化治理与自动化 Keepers**，实现了一个 **高度自动化、可审计、可组合** 的链上体育预测平台。
