...
/**
 * @title SimulateBets_V3
 * @notice 模拟多用户多市场下注（V3 版本）
 * @dev 通过 BettingRouter_V3 下注，符合 V3 权限设计
 *
 * 使用方法：
 *   1. 确保已运行 V3 部署脚本，并生成了 deployments/localhost_v3.json
 *   2. 脚本会自动从配置文件读取 Factory、USDC 和 Router 地址
 *   3. 设置环境变量（可选）：
 *      export NUM_BETTORS=9          # 下注用户数量
 ...
 *   4. 运行脚本：
 *      PRIVATE_KEY=0x... forge script script/SimulateBets_V3.s.sol:SimulateBets_V3 \
 *        --rpc-url http://localhost:8545 --broadcast
 *
 * V3 权限设计：
 *   - 用户通过 BettingRouter_V3 下注
 *   - Router 拥有市场的 ROUTER_ROLE
 *   - 用户只需授权 USDC 给 Router
 */
contract SimulateBets_V3 is Script {
    // Deployment JSON 路径
    ...
    function run() external {
        console.log("\n========================================");
        console.log("  Simulate Multi-User Betting (V3)");
        console.log("========================================\n");
        ...
        uint256 numBettors = vm.envOr("NUM_BETTORS", uint256(9));
        ...
        BettingRouter_V3 router = BettingRouter_V3(routerAddr);
        ...
        if (marketCount == 0) {
            console.log("No markets found. Please run CreateAllMarketTypes_V3.s.sol first.");
            return;
        }

        // 构建可用市场列表（状态为 Open 的 V3 市场）
        ...
        for (uint256 i = 0; i < marketCount; i++) {
            ...
            // 尝试读取 V3 市场状态
            try Market_V3(marketAddr).status() returns (IMarket_V3.MarketStatus status) {
                // V3 市场状态：Open = 1
                if (status == IMarket_V3.MarketStatus.Open || !skipLocked) {
        ...
        console.log("Available V3 Markets:", availableCount);
        if (availableCount == 0) {
            console.log("No available V3 markets for betting.");
            return;
        }
        console.log("");
        ...
        console.log("========================================");
        console.log("Simulating Bets via Router...");
        console.log("========================================\n");

        // 为每个用户模拟下注
        // 跳过账户 #0（管理员），使用账户 #1-9 下注
        for (uint256 u = 1; u <= numBettors && u < testPrivateKeys.length; u++) {
            ...
            for (uint256 b = 0; b < betsPerUser; b++) {
                ...
                // 检查市场状态
                IMarket_V3.MarketStatus status;
                try market.status() returns (IMarket_V3.MarketStatus _status) {
                    status = _status;
                } catch {
                ...
                // V3 市场状态检查：Open = 1
                if (status != IMarket_V3.MarketStatus.Open) {
                    if (skipLocked) {
                        stats.skippedMarkets++;
                        continue;
                    }
                ...
                try router.placeBet(marketAddr, outcomeId, betAmount, 0) returns (uint256 shares) {
                    ...
                    if (lowLevelData.length >= 4) {
                        ...
                        console.log("  Bet FAILED: Custom error selector");
        ...
        // ========================================
        console.log("========================================");
        console.log("  Betting Simulation Summary (V3)");
        console.log("========================================");
