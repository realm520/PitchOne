...
/**
 * API 路由：代理 GraphQL 请求到 Graph Node
 *
...
export async function GET() {
  return NextResponse.json({
    message: 'GraphQL Proxy API (Admin)',
    endpoint: 'POST /api/subgraph/subgraphs/name/pitchone-local',
    graphNodeUrl: process.env.GRAPH_NODE_URL || 'http://127.0.0.1:8010/subgraphs/name/pitchone-local',
    status: 'OK',
  });
}
